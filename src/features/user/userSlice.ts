...
export const fetchUser = createAsyncThunk(
  'user/fetchUser',
  async (authData: firebase.User) => {
    const user = await getUser(authData);
    if (!user) {
      const addedUser = await addUser(authData);
      return addedUser
    } else {
      // The value we return becomes the `fulfilled` action payload
      return user
...
export const selectUserPhoto = (state: RootState) => state.user.userData?.photo || null;
export const selectUID = (state: RootState) => state.user.uid || null;
